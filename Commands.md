...
```bash
docker network create proudlyzimmart_net

docker stop $(docker ps -q)

rm -rf proudlyzimmart-api
...
```bash
docker compose run web wagtail start proudlyzimmart .
...
sudo git clone https://github.com/kelvinmaringire/proudlyzimmart-api.git

cd proudlyzimmart-api
...
bash scripts/restore.sh proudlyzimmart_backup_20240202_020000.zip
...
**Restore Process:**
- Media files: Automatically copied to `proudlyzimmart_web` container
...
0 2 * * * cd /home/user/srv/proudlyzimmart-api && bash scripts/backup.sh >> backup.log 2>&1

# Or with custom environment variables
0 2 * * * cd /home/user/srv/proudlyzimmart-api && RCLONE_REMOTE=gdrive RCLONE_PATH=RuzivoflowBackups bash scripts/backup.sh >> backup.log 2>&1
...
# Backup Media Files
mkdir -p ./media
docker cp proudlyzimmart_web:/app/media/. ./media

# Restore Media Files
docker cp ./media/. proudlyzimmart_web:/app/media/
```
...
```bash
# Backup Database
docker compose exec -T db pg_dump -U postgres -d proudlyzimmart_db -F c > ./data_backup.dump

# Restore Database
docker compose exec -T db psql -U postgres -c "DROP DATABASE proudlyzimmart_db;"
docker compose exec -T db psql -U postgres -c "CREATE DATABASE proudlyzimmart_db;"
docker compose exec -T db pg_restore -U postgres -d proudlyzimmart_db < data_backup.dump
```
